...
function renderHighlightDialogue(){
  ...
  if(!dialogue)return;
  const participants=new Map((dialogue.participants||[]).map(item=>[item.id,item]));
  const script=(dialogue.messages||[]).map(message=>message.type==="overlay"?`<li class="is-overlay"><span>状态 / 内心 OS</span>${escapeHtml(message.text)}</li>`:`<li><b>${escapeHtml(participants.get(message.speaker_id)?.display_name||"人物")}：</b>${escapeHtml(message.text)}</li>`).join("");
  card.innerHTML=`<div><header><span>${dialogue.chat_type==="group"?"群聊":"私聊"}</span><b>精华对话 · 仅文字预览</b></header><h2>${escapeHtml(dialogue.chat_title||dialogue.title||"精华对话")}</h2><p>${escapeHtml(dialogue.scene_summary||"从原作提取的高张力连续对话。")}</p></div><ol class="dialogue-script">${script}</ol><p class="dialogue-export-note">聊天截图、视频和视频 BGM 请在最后的“导出素材”页面制作。</p>`;
}
...
function chatLayout(ctx,dialogue,visibleFloat){
  ...
  const entries=(dialogue.messages||[]).slice(0,Math.ceil(visibleFloat)).map((message,index)=>{
    const overlay=message.type==="overlay",person=overlay?null:participants.get(message.speaker_id)||{display_name:"角色",side:index%2?"right":"left"},lines=canvasTextLines(ctx,message.text,overlay?760:610),bubbleWidth=Math.min(overlay?840:700,Math.max(170,...lines.map(line=>ctx.measureText(line).width+(overlay?64:72)))),bubbleHeight=lines.length*(overlay?49:55)+(overlay?32:42);
    return {message,person,overlay,lines,bubbleWidth,bubbleHeight,index,height:bubbleHeight+(overlay?34:54)};
  });
...
function drawChatCanvas(canvas,dialogue,visibleFloat=(dialogue?.messages||[]).length){
  ...
  layout.entries.forEach((item,index)=>{
    ...
    if(item.overlay){const bubbleX=(width-item.bubbleWidth)/2,bubbleY=item.y+10+(1-alpha)*18;ctx.save();ctx.globalAlpha=alpha*.9;roundedPath(ctx,bubbleX,bubbleY,item.bubbleWidth,item.bubbleHeight,22);ctx.fillStyle="rgba(45,66,79,.68)";ctx.fill();ctx.fillStyle="#fff";ctx.textAlign="center";ctx.textBaseline="top";ctx.font='30px "Microsoft YaHei"';item.lines.forEach((line,lineIndex)=>ctx.fillText(line,width/2,bubbleY+17+lineIndex*49));ctx.restore();return;}
    const right=item.person.side==="right",avatarX=right?990:90,bubbleX=right?width-145-item.bubbleWidth:145,bubbleY=item.y+28+(1-alpha)*24,personIndex=Math.max(0,(dialogue.participants||[]).findIndex(person=>person.id===item.message.speaker_id)),avatarColors=["#4e9db4","#7459a7","#cf7656","#4c8a68","#a35f83","#56738f"];
    ctx.save();ctx.globalAlpha=alpha;ctx.beginPath();ctx.arc(avatarX,item.y+66,42,0,Math.PI*2);ctx.fillStyle=right?"#f5c927":avatarColors[personIndex%avatarColors.length];ctx.fill();ctx.fillStyle=right?"#4b3b00":"#fff";ctx.textAlign="center";ctx.font='700 28px "Microsoft YaHei"';ctx.fillText([...String(item.person.display_name||"角")][0],avatarX,item.y+67);
...
function renderChatExport(bgmAvailable=false){const card=$("#chatExportCard"),dialogue=state.analysis?.highlight_dialogue;if(!card)return;card.hidden=!dialogue;if(!dialogue)return;const seconds=dialogue.target_duration_seconds||15;$("#chatExportTitle").textContent=dialogue.chat_title||"聊天记录";$("#chatDurationLabel").textContent=`视频约 ${seconds} 秒`;$("#chatExportSummary").textContent=dialogue.scene_summary||"从原作提取的高张力连续对话";drawChatCanvas($("#chatCanvas"),dialogue);$("#chatBgmInput").disabled=!bgmAvailable;$("#chatBgmInput").checked=bgmAvailable;$("#chatBgmHint").textContent=bgmAvailable?"已使用本次生成的配乐，可取消":"本次没有可用 BGM，将生成无声视频";$("#buildChatVideoBtn").textContent=`生成约 ${seconds} 秒聊天视频`;$("#chatVideoState").textContent="时长按消息数量、文字长度和状态描述自动计算。";}
...
async function buildChatVideo(){
  ...
  const button=$("#buildChatVideoBtn"),seconds=Math.min(25,Math.max(10,Number(dialogue.target_duration_seconds)||15)),duration=seconds*1000,canvasStream=canvas.captureStream(24),useBgm=$("#chatBgmInput")?.checked&&audioServerUrl,mime=["video/webm;codecs=vp9,opus","video/webm;codecs=vp8,opus","video/webm"].find(type=>MediaRecorder.isTypeSupported(type))||"",chunks=[];let context=null,source=null,stream=canvasStream;
  setBusy(button,true,"正在生成 0%");$("#chatVideoState").textContent=`正在实时合成约 ${seconds} 秒${useBgm?"带 BGM":"无声"}视频，请保持页面打开。`;
  try{
    if(useBgm){const audioData=await fetch(audioServerUrl).then(response=>{if(!response.ok)throw new Error("BGM 读取失败");return response.arrayBuffer();});context=new (window.AudioContext||window.webkitAudioContext)();const decoded=await context.decodeAudioData(audioData.slice(0)),destination=context.createMediaStreamDestination();source=context.createBufferSource();source.buffer=decoded;source.connect(destination);stream=new MediaStream([...canvasStream.getVideoTracks(),...destination.stream.getAudioTracks()]);}
    const recorder=new MediaRecorder(stream,mime?{mimeType:mime,videoBitsPerSecond:4500000}:undefined);recorder.ondataavailable=event=>{if(event.data.size)chunks.push(event.data);};const finished=new Promise((resolve,reject)=>{recorder.onstop=resolve;recorder.onerror=()=>reject(recorder.error||new Error("视频编码失败"));});recorder.start(1000);if(source)source.start();const started=performance.now(),count=dialogue.messages.length;
    ...
    recorder.stop();if(source)try{source.stop();}catch{}await finished;stream.getTracks().forEach(track=>track.stop());if(context)await context.close();const blob=new Blob(chunks,{type:mime||"video/webm"});if(!blob.size)throw new Error("视频文件为空");if(chatVideoObjectUrl)URL.revokeObjectURL(chatVideoObjectUrl);chatVideoObjectUrl=URL.createObjectURL(blob);
    const video=$("#chatVideoPreview"),link=$("#downloadChatVideoLink");video.src=chatVideoObjectUrl;video.hidden=false;link.href=chatVideoObjectUrl;link.download=`${safeName(state.project.title)}-精华对话-${seconds}秒.webm`;link.hidden=false;$("#chatVideoState").textContent=`约 ${seconds} 秒聊天视频已生成${useBgm?"，包含 BGM":"，无声"}。`;
  }catch(error){if(context)await context.close().catch(()=>{});toast(`聊天视频生成失败：${error.message}`);$("#chatVideoState").textContent="视频没有生成成功，请重试。";}finally{setBusy(button,false);button.textContent=`重新生成约 ${seconds} 秒聊天视频`;}
}
...
function renderResult() {
  ...
  renderChatExport(music?.status === "succeeded" && Boolean(task.bundle?.audio_url));
