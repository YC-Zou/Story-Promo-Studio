...
服务端会检查枚举、数量、三个类型/策略互异、计划一致性、行长、证据引用并重算选择分与候选排名分。候选还要经过独立全文事实与剧透审查；每次定向修订稿也必须立即复审通过才可替换。未配置密钥时返回 503，不生成或返回模拟候选。
...
无论候选是否被修改，接口都会重新生成 `selected_lines`、检查 20—300 个非空白字符、事实证据与剧透边界，再返回 `selected_hook_profile` 和 `material_recommendation`。校验失败返回 422，并在 `details` 中给出问题句、代码和原因。

## 底图、生成与任务

- `GET /api/backgrounds?category=horror_rules`：对每个合法主类别只返回 3 个不可编辑的 `BackgroundPreset`。
- `POST /api/projects/:id/generate`：输入 `selected_type=comic|card`；card 必须带属于当前主类别的 `background_id`。只有已确认有效钩子可以生成。
...
浏览器用服务端返回的冻结计划生成预览，在导出前按指定 `task_id` 校验归属，再生成 PNG 与无压缩 ZIP并交给服务端提供真实下载。B 类把全部 `selected_lines` 按原顺序合并为 2—6 组，不增删改字；A 类按原顺序分配至 2—8 页。`project.json` 保存事实引用、确认钩子、类型、任务和适配器模式，不保存密钥。
