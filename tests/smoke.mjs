...
const analyzed = await post("/api/projects/analyze", { title:sample.title, author:"测试作者", source_url:"https://www.zhihu.com/question/1/answer/2", authorized:true, body:sample.body, labels:sample.labels });
...
const task = await post(`/api/projects/${id}/generate`, { selected_type:"card", background_id:bg.id, simulate_failure:"music" }, 202);
await post(`/api/projects/${id}/generate`, { selected_type:"card", background_id:bg.id }, 409);
