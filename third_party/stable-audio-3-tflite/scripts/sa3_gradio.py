...
def verbose_basename(prompt, negative_prompt, cfg, sigma_max, seed, precision) -> str:
    ...
    if precision and precision != "fp32":
        base += f".{precision}"
    # Keep the full output path below the legacy Windows MAX_PATH boundary.
    # A short digest preserves uniqueness after truncation.
    digest = hashlib.sha1(base.encode("utf-8")).hexdigest()[:10]
    return f"{base[:72]}.{digest}.{seed}"
...
def get_decoder(name: str, precision: str) -> RungDecoder:
    ...
    # The official small bundle intentionally ships a fp32 DiT with the
    # quality-preserving w8a8 SAME codec. Prefer that installed codec instead
    # of attempting an unavailable fp32 codec download in offline mode.
    if cprec == "fp32" and not (REPO / dec_rel(name, cprec)).exists() \
            and (REPO / dec_rel(name, "w8a8")).exists():
        cprec = "w8a8"
    key = (name, cprec)
...
def get_encoder(name: str, precision: str) -> RungEncoder:
    cprec = _codec_prec(precision)
    if cprec == "fp32" and not (REPO / enc_rel(name, cprec)).exists() \
            and (REPO / enc_rel(name, "w8a8")).exists():
        cprec = "w8a8"
    key = (name, cprec)
