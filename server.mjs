...
const runtimeDir = resolve(root, "work", "runtime");
...
async function restoreState() {
  ...
  try {
    ...
    for (const [id, project] of saved.projects || []) projects.set(id, project);
...
async function analyzeProject(input) {
  if (!String(input.title || "").trim()) throw error("缺少作品名");
  if (!String(input.author || "").trim()) throw error("缺少作者名");
  if (!input.authorized) throw error("未确认作品授权");
...
function recommendationFor(text, hookType) {
  ...
  let recommended = comicScore - cardScore >= 10 ? "comic" : cardScore - comicScore >= 10 ? "card" : ["relationship_tension", "crisis_choice", "emotional_scene"].includes(hookType) ? "comic" : "card";
...
async function runMaterial(task) {
  ...
  if(task.bundle.selected_type==="card"){task.bundle.image_assets.forEach(asset=>asset.status="succeeded");material.status="succeeded";material.message="固定预设底图与冻结短句已合成";return finalizeTask(task);}
  if (!imageApiKey) { material.status = "failed"; material.message = "Seedream 未配置，已阻止用模拟画布冒充生成结果"; return finalizeTask(task); }
  const errors = [];
  for (const [index, asset] of task.bundle.image_assets.entries()) {
    ...
    for (let attempt = 0; attempt < 2 && !success; attempt++) try {
      ...
      else if (data?.url) { const upstream=await fetch(data.url,{signal:AbortSignal.timeout(30000)});if(!upstream.ok)throw new Error(`Seedream 图片下载失败（${upstream.status}）`);asset._image_buffer=Buffer.from(await upstream.arrayBuffer()); }
    ...
    } catch (err) { lastError=err; upstreamState.image.verified=false; upstreamState.image.last_error=err?.message || "未知错误"; }
    if (!success) { asset.status="failed"; errors.push(`${asset.file_name}: ${lastError?.message || "未知错误"}`); }
  }
  if (errors.length) { material.status="failed"; material.message=`${errors.length} 页在自动重试后仍失败；重试任务只会重跑失败页：${errors.join("；")}`; }
  else { material.status = "succeeded"; material.message = `Seedream 已完成 ${task.bundle.image_assets.length} 张底图，文案由本地画布叠加`; }
  finalizeTask(task);
}
async function runMusic(task) {
  const music = task.subtasks.find(item => item.id === "music"); let lastError;
  for (let attempt = 0; attempt < 2; attempt++) {
    try { const profile=task.bundle.music_profile;const generated = await callLocalMusic(profile.prompt,profile.negative_prompt,profile.duration,profile.steps,profile.cfg,profile.seed + attempt); task.bundle._audio_buffer = generated.buffer; task.bundle._audio_file=await saveMedia(task.id,"bgm.wav",generated.buffer); task.bundle.audio_url = `/api/tasks/${task.id}/audio`; music.status = "succeeded"; music.message = `gpt-5.6-sol 提示词 + 本机 SA3 已生成并通过时长、静音与爆音检查的 WAV${attempt ? "（换 seed 重试成功）" : ""}`; finalizeTask(task); return; }
    catch (err) { lastError = err; }
  }
  music.status = "failed"; music.message = `本机音乐生成失败，已自动重试一次：${lastError?.message || "未知错误"}`; finalizeTask(task);
}
async function callLocalMusic(prompt, negativePrompt, seconds, steps, cfg, seed) {
  ...
  const started = await fetch(`${musicBaseUrl}/gradio_api/call/generate`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({data}), signal:AbortSignal.timeout(10000) });
  ...
  const stream = await fetch(`${musicBaseUrl}/gradio_api/call/generate/${eventId}`, { signal:AbortSignal.timeout(240000) }); if (!stream.ok) throw new Error(`Gradio 任务失败（${stream.status}）`); const text = await stream.text();
  ...
  const audio=await fetch(value,{signal:AbortSignal.timeout(30000)});if(!audio.ok)throw new Error(`WAV 下载失败（${audio.status}）`);const buffer=Buffer.from(await audio.arrayBuffer());validateWav(buffer);return {url:value,buffer};
}
...
function finalizeTask(task) {
  const states = task.subtasks.map(item => item.status); if (states.some(s => ["queued","running"].includes(s))) { task.status="running"; task.progress = states.filter(s=>s==="succeeded").length * 45 + 35; task.updated_at=Date.now(); persistState(); return; }
  const material = task.subtasks.find(item=>item.id==="material"), music=task.subtasks.find(item=>item.id==="music");
  task.status = material.status === "failed" ? "failed" : music.status === "failed" ? "partially_failed" : "succeeded"; task.progress=100; task.message = task.status === "succeeded" ? "全部物料已完成" : task.status === "partially_failed" ? "图片已完成，BGM 失败；仍可导出图片包" : "视觉物料失败，不能导出残缺结果"; task.updated_at=Date.now(); persistState();
}
function publicTask(task) { const copy = JSON.parse(JSON.stringify(task, (key, value) => key.startsWith("_") ? undefined : value)); const project=projects.get(task.project_id); if(!copy.bundle.project_metadata&&project)copy.bundle.project_metadata={title:project.input.title,author:project.input.author,source_url:project.input.source_url}; delete copy.bundle.audio_source_url; return copy; }
...
const httpServer=createServer(async (req, res) => {
  ...
  try {
    if (req.method === "GET" && pathname === "/api/health") return sendJson(res, 200, { ok:true, mode: textApiKey ? "live_unverified" : "unconfigured", text_model: { ...upstreamState.text, model:hookModel, base_url:textBaseUrl }, image_model:{ ...upstreamState.image, model:imageModel, endpoint:"/images/generations", base_url:imageBaseUrl }, music:{ reachable:await musicReachable(), provider:"Stable Audio 3 Small Music", base_url:musicBaseUrl }, limits:{body_non_whitespace:50000} });
    if (req.method === "GET" && pathname === "/api/samples") { const samples=await loadSamples(); return sendJson(res,200,{samples:samples.map(({work_id,title,labels,data_no})=>({work_id,title,labels,data_no}))}); }
    const sampleMatch = pathname.match(/^\/api\/samples\/([^/]+)$/); if (req.method === "GET" && sampleMatch) { const sample=(await loadSamples()).find(item=>String(item.work_id)===sampleMatch[1]); if(!sample)throw error("授权样例不存在",404); return sendJson(res,200,{work_id:sample.work_id,title:sample.title,labels:sample.labels,body:sample.body}); }
    ...
    const selectedExportMatch = pathname.match(/^\/api\/projects\/([^/]+)\/hooks\/selected\/export$/); if(req.method==="GET"&&selectedExportMatch){const project=projects.get(selectedExportMatch[1]);if(!project)throw error("项目不存在或服务已重启，请重新分析",404);if(project.selectedHook?.validation_status!=="passed")throw error("尚未确认有效钩子",409);const text=project.selectedHook.final_text;res.writeHead(200,{"Content-Type":"text/plain; charset=utf-8","Content-Disposition":`attachment; filename="selected-hook.txt"`,"Content-Length":Buffer.byteLength(text)});return res.end(text);}
  ...
  } catch (err) { if(pathname.startsWith("/api/"))return sendJson(res,err.statusCode||500,{error:err.message||"服务异常",details:err.details});res.writeHead(404,{"Content-Type":"text/plain; charset=utf-8"});res.end("Not found"); }
});
httpServer.requestTimeout=20*60*1000;
httpServer.headersTimeout=20*60*1000+5000;
httpServer.listen(port,"127.0.0.1",()=>{console.log(`Zhihu Story Workbench: http://127.0.0.1:${port}`);console.log(`Hook model: ${textApiKey ? `${hookModel} (configured, awaiting verified call)` : "UNCONFIGURED"}`);console.log(`Seedream: ${imageApiKey ? `${imageModel} (configured, awaiting verified call)` : "UNCONFIGURED"}`);console.log(`Music adapter: ${musicBaseUrl}`);});
